...
export async function generatePrediction(req, res) {
  try {
    const { mine } = req.body;
    if (!mine) return res.status(400).json({ message: "mine is required" });
    const [geo, prod] = await Promise.all([
      GeologicalData.find({ mine }),
      Production.find({ mine }).sort({ date: -1 }).limit(90),
    ]);
    const p = predictProduction(prod);
    const reserve = estimateReserve(geo);
    const doc = await Prediction.create({
      mine,
      predictionDate: new Date(),
      predictedReserveTons: reserve,
      predictedProductionTons: p.predicted,
      shortfallRisk: p.risk,
      shortfallTons: p.shortfall,
      riskFactors: p.factors,
      recommendations: recommendations(p.factors),
      modelVersion: "baseline-v1",
      inputSummary: { geologyRows: geo.length, productionRows: prod.length },
    });
...
export async function dashboard(req, res) {
  try {
    ...
    const latest = await Prediction.findOne({ mine }).sort({ createdAt: -1 });
...
export async function chat(req, res) {
  try {
    ...
    if (!mine || !question?.trim())
      return res
        .status(400)
        .json({ message: "mine and question are required" });
...
export async function analyzeLiveLocation(req, res) {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "mine, latitude, and longitude are required" });
    const [production, geology] = await Promise.all([
      Production.find({ mine }).sort({ date: 1 }).limit(365),
      GeologicalData.find({ mine }).sort({ createdAt: -1 }).limit(365),
    ]);
