export async function analyzeLocation({ mine, latitude, longitude, production, geology }) {
  ...
  return {
    ...
    suitabilityStatus: ['SUITABLE', 'SUITABLE_WITH_REVIEW', 'NOT_SUITABLE'].includes(result.suitabilityStatus) ? result.suitabilityStatus : 'SUITABLE_WITH_REVIEW',
    suitabilityReason: result.suitabilityReason || 'A qualified site review is required before work begins.',
    ...
    riskFactors: Array.isArray(result.riskFactors) ? result.riskFactors.slice(0, 5) : [],
    ...
    disclaimer: result.disclaimer || 'AI screening only; confirm with a qualified field survey.',
  };
}